...
for i in items: #in items
    ...
    dados_treino_item_i = dados_treino.loc[(dados_treino.item == i)].copy()
    if ((i == 1) or (i % 5 == 0)):
        print(f"Dados de treino do item {i:02}:")
        display(dados_treino_item_i)

    # Caminho e nome arquivo onde o modelo será salvo/carregado
    model_path = f'Modelos\\item{i:02}.joblib'

    # Carregar o modelo se ele já estiver salvo, caso contrário treinar e salvar
    model = load_model(model_path)
    if model is None:
        print(f"Treinando o modelo para o item {i:02}...")
        model = train_and_save_model(dados_treino_item_i, model_path)
    ...
    forecast_path = f'Predicoes\\pred_item_{i:02}.csv'
    if not os.path.exists(forecast_path):
        print(f"Arquivo de predicao do item {i:02} sera gerado")
        dates = pd.date_range(start=start_date, end=end_date)
        ...
        print(f"Arquivo de predicao do item {i:02} ja existe!")
    ...
    with open(f'Predicoes\\pred_item_{i:02}.csv', 'r') as pred_item_csv:

        # Crie um leitor de CSV com cabeçalho
        leitor_pred_item_csv = csv.DictReader(pred_item_csv)

        #pred_item_csv['ds'] = pd.to_datetime(pred_item_csv['ds'])           

        for linha_csv in leitor_pred_item_csv: # Iterando sobre as linhas do arquivo CSV
            for linha_timeframe_df in dict_timeframe_df:
                linha_csv['ds'] = pd.to_datetime(linha_csv['ds'])
                if (linha_timeframe_df['ds'] == linha_csv['ds']) and (linha_timeframe_df['item'] == i):
